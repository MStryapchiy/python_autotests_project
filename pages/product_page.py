...
class ProductPage(BasePage):
    def should_be_add_to_basket_button(self):
        assert self.is_element_present(*ProductPageLocators.ADD_TO_BASKET_LINK), "Add to basket button is not existed"
    ...
    def should_be_product_price_in_messege(self):
        assert self.get_product_price_on_product_page() == self.get_product_price_in_message(), "There is no product price in message" 
    ...
    def should_be_product_name_in_messege(self):
        assert self.get_product_name_on_product_page() == self.get_product_name_in_message(), "There is no product name in message"
